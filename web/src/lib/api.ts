...
class ApiService {
  ...
  // Quote methods
  // Streak endpoint
  async getStreak(): Promise<{ streak: number; last_entry_date?: string }> {
    const response = await fetch(`${API_BASE_URL}/records/streak`, {
      headers: this.getAuthHeaders(),
    });

    const result = await this.handleResponse<{ streak: number; last_entry_date?: string }>(response);
    return result.data!;
  }

  async getRandomQuote(): Promise<Quote> {    const response = await fetch(`${API_BASE_URL}/quotes/random`, {
      headers: this.getAuthHeaders(),
    });
